...
import { getProjectStatusReport } from "@/modules/reports/services/report.service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await getProjectStatusReport(filters);
    return NextResponse.json(serializePrisma(result));
  } catch (err) {
    console.error("[reports/project-status]", err);
