...
import { getTaskCompletionReport } from "@/modules/reports/services/report.service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await getTaskCompletionReport(filters);
    return NextResponse.json(serializePrisma(result));
  } catch (err) {
    console.error("[reports/task-completion]", err);
