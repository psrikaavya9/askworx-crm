...
import { CheckCircle, XCircle, Receipt, ExternalLink, AlertTriangle } from "lucide-react";
...
export function AccountsExpenses({ data, onPageChange }: Props) {
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-md">
        {data.data.length === 0 ? (
          <EmptyState
            ...
            title="All clear"
            description="No expenses pending accounts review."
          />
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {data.data.map((expense) => (
                <tr key={expense.id} className="bg-white transition-colors hover:bg-gray-50">
                  <td className="px-5 py-4 font-medium text-gray-900">{staffName(expense)}</td>
                  ...
                  <td className="px-5 py-4">
                    {expense.isFlagged ? (
                      <span
                        title={expense.flagReason ?? "Flagged for review"}
