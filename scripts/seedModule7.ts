...
/**
 * ============================================================================
 * seedModule7.ts — HR Vault + Video + Quiz Test Data Seeder (TypeScript)
 * ASKworX CRM Platform
 * ============================================================================
 *
 * Usage (from project root):
 *   npx ts-node --skip-project scripts/seedModule7.ts            # seed all
 *   npx ts-node --skip-project scripts/seedModule7.ts --clean    # delete seed data
 *   npx ts-node --skip-project scripts/seedModule7.ts --reset    # clean + re-seed
 *   npx ts-node --skip-project scripts/seedModule7.ts --verify   # counts only
 *
 * Seed markers:
 *   • HrDocument / HrVideo  → uploadedBy = 'seed-script'
 *   • Staff                 → email ending in '@vault-seed.test'
 *   • DocAcknowledgement / VideoWatchLog / VideoQuizAttempt → staffId LIKE 'seed-%'
 * ============================================================================
 */
...
type DocumentCategory = "POLICY" | "CONTRACT" | "HANDBOOK" | "FORM" | "SOP" | "COMPLIANCE" | "OTHER";
type DocumentStatus   = "ACTIVE" | "ARCHIVED" | "EXPIRED";
type AccessLevel      = "ALL" | "MANAGER_ONLY" | "HR_ONLY" | "CUSTOM";
type VideoCategory    = "ONBOARDING" | "TRAINING" | "SAFETY" | "COMPLIANCE" | "GENERAL";
type VideoStatus      = "PROCESSING" | "READY" | "FAILED";
type StaffRole        = "ADMIN" | "MANAGER" | "STAFF";
type WarningLevel     = "none" | "low" | "medium" | "high";
type AnswerStrategy   = "correct" | "wrong" | "mostly-correct" | "half";
...
const daysFrom   = (n: number): string => new Date(now.getTime() + n * 86_400_000).toISOString();
const randDaysAgo = (max: number): string => daysAgo(Math.floor(Math.random() * max));

// ─── Fixed IDs ───────────────────────────────────────────────────────────────

// All IDs use only hex characters so they are valid PostgreSQL UUIDs.
// Prefix scheme: dc = document, b1d = video, acc = quiz, abe = question

const DOC_IDS: string[] = Array.from({ length: 16 }, (_, i) =>
  `dc${String(i + 1).padStart(6, "0")}-0000-4000-a000-000000000000`
);

const VID_IDS: string[] = Array.from({ length: 9 }, (_, i) =>
  `b1d0${String(i + 1).padStart(4, "0")}-0000-4000-b000-000000000000`
);

const QUIZ_IDS = {
  fireSafety:      "acc00001-0000-4000-a000-000000000000",
  workplaceEthics: "acc00002-0000-4000-a000-000000000000",
  dataProtection:  "acc00003-0000-4000-a000-000000000000",
} as const;

const qq = (quizIdx: number, qIdx: number): string =>
  `abe${String(quizIdx).padStart(5, "0")}-${String(qIdx).padStart(4, "0")}-4000-a000-000000000000`;

// ─── Staff ────────────────────────────────────────────────────────────────────

const STAFF: Record<string, StaffDef> = {
...
// IMPORTANT: Use https:// — http:// is blocked by modern browsers as mixed content
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        options: ["Consent", "Legitimate interests", "Business curiosity", "Legal obligation"],
        correctIdx: 2,
...
function computeWarningLevel(status: DocumentStatus, expiresAt: string | null): WarningLevel {
  if (status !== "ACTIVE" || !expiresAt) return "none";
...
function buildAnswers(
  ...
  questions.forEach((q, i) => {
    if (strategy === "correct") {
      ...
      // half: even-indexed correct, odd-indexed wrong
...
function calcScore(quiz: QuizDef, answers: Record<string, number>): number {
  const correct = quiz.questions.reduce(
    (acc, q) => acc + (answers[q.id] === q.correctIdx ? 1 : 0),
    0
  );
...
async function cleanSeedData(): Promise<void> {
  ...
  log.ok(`Deleted ${aq.rowCount} quiz attempts`);

  const quizPlaceholders = Object.values(QUIZ_IDS).map((_, i) => `$${i + 1}`).join(",");
  const qq2 = await pool.query(
    `DELETE FROM "VideoQuiz" WHERE id IN (${quizPlaceholders}) RETURNING id`,
    Object.values(QUIZ_IDS)
  );
  log.ok(`Deleted ${qq2.rowCount} quizzes (questions cascade)`);
  ...
  const vidP = VID_IDS.map((_, i) => `$${i + 1}`).join(",");
  const vids = await pool.query(
    `DELETE FROM "HrVideo" WHERE id::text IN (${vidP}) RETURNING id`,
    VID_IDS
  );
  log.ok(`Deleted ${vids.rowCount} videos`);

  const docP = DOC_IDS.map((_, i) => `$${i + 1}`).join(",");
  const docs = await pool.query(
    `DELETE FROM "HrDocument" WHERE id::text IN (${docP}) RETURNING id`,
    DOC_IDS
  );
  log.ok(`Deleted ${docs.rowCount} documents`);

  const staff = await pool.query(
    `DELETE FROM "Staff" WHERE email LIKE $1 RETURNING id`,
    [`%${SEED_EMAIL_SUFFIX}`]
  );
...
async function seedStaff(client: PoolClient): Promise<void> {
  ...
  for (const s of Object.values(STAFF)) {
    ...
    await client.query(
      `INSERT INTO "Staff"
         (id, "firstName", "lastName", email, phone, role, department, status, "createdAt", "updatedAt")
...
async function seedDocuments(client: PoolClient): Promise<void> {
  ...
  for (const doc of DOCUMENTS) {
    const status: DocumentStatus = doc.status ?? "ACTIVE";
    const warningLevel = computeWarningLevel(status, doc.expiresAt);
    ...
    log.ok(`[${badge}] ${doc.title.substring(0, 55)}`);
    created++;
  }
...
async function seedAcknowledgements(client: PoolClient): Promise<void> {
  ...
  const ackMatrix: Array<{ staffId: string; docIds: string[] }> = [
    ...
    { staffId: STAFF.emp2.id,  docIds: [ackDocs[0]?.id].filter((x): x is string => !!x) },
    // emp3, emp4, emp5: no acks — used to test "pending" state
  ];

  let created = 0;
  for (const { staffId, docIds } of ackMatrix) {
    for (const docId of docIds) {
      await client.query(
        `INSERT INTO "DocAcknowledgement" (
           id, "documentId", "staffId", "acknowledgedAt",
           "ipAddress", "userAgent", signature, notes,
           "isDeleted", "createdAt", "updatedAt"
         ) VALUES (
           gen_random_uuid(), $1, $2, $3,
           '192.168.1.100', 'Mozilla/5.0 (Seed)', $4,
           'Acknowledged via seed script',
           false, NOW(), NOW()
         )
         ON CONFLICT ("documentId", "staffId") DO NOTHING`,
        [docId, staffId, randDaysAgo(14), TINY_PNG]
      );
      created++;
    }
  }
  ...
  log.dim("emp3, emp4, emp5 have NO acknowledgements (test pending state)");
}

// ─── Seed Videos ─────────────────────────────────────────────────────────────

async function seedVideos(client: PoolClient): Promise<void> {
  log.section("Seeding Videos (9 records)…");
  let created = 0;

  for (let i = 0; i < VIDEOS.length; i++) {
    const v      = VIDEOS[i];
    const srcUrl = SAMPLE_VIDEO_URLS[i] ?? SAMPLE_VIDEO_URLS[0];
    ...
    await client.query(
      ...
      [
        ...
        v.category,
        srcUrl,
        v.status === "READY" ? srcUrl : null,
        `vault/videos/seed_${v.id}`,
        500 * 1024 * 1024,   // 500 MB placeholder
        v.duration,
        v.status,
        v.accessLevel,
        `{${v.tags.join(",")}}`,
    ...
    const req    = v.isRequired ? "⭐ Required" : "          ";
    const status = v.status === "READY" ? "READY     " : "⏳ PROCESS";
    log.ok(`[${status}] ${req} ${v.title.substring(0, 48)}`);
    created++;
  }
...
async function seedWatchLogs(client: PoolClient): Promise<void> {
  log.section("Seeding Video Watch Logs…");

  interface WatchEntry { idx: number; pct: number }
  const scenarios: Array<{ staffId: string; videos: WatchEntry[] }> = [
    ...
    { staffId: STAFF.emp2.id, videos: [
      { idx: 0, pct: 15  },   // Onboarding — barely watched
      { idx: 1, pct: 0   },   // Fire Safety — 0 %
    ]},
  ...
  let created = 0;
  for (const { staffId, videos } of scenarios) {
      ...
      await client.query(
        `INSERT INTO "VideoWatchLog" (
         ...
         ON CONFLICT ("videoId", "staffId") DO UPDATE SET
           ...
           "updatedAt"      = NOW()`,
        [v.id, staffId, watched, dur, pct, completed, randDaysAgo(7)]
      );
      created++;
    }
  }
...
async function seedQuizzes(client: PoolClient): Promise<void> {
  log.section("Seeding Quizzes…");
  ...
  for (const quiz of QUIZZES) {
    ...
    for (const q of quiz.questions) {
      await client.query(
        `INSERT INTO "VideoQuizQuestion"
           (id, "quizId", question, options, "correctIdx", explanation, "order")
        ...
        [q.id, quiz.id, q.question, JSON.stringify(q.options), q.correctIdx, q.explanation, q.order]
      );
    }

    const videoTitle = VIDEOS.find((v) => v.id === quiz.videoId)?.title ?? quiz.videoId;
    log.ok(`Quiz: "${videoTitle.substring(0, 40)}" — ${quiz.questions.length} questions, pass ${quiz.passMark}%`);
  }
  ...
  const attempts: AttemptDef[] = [
    { quizId: QUIZ_IDS.fireSafety,      videoId: VID_IDS[1], staffId: STAFF.emp1.id,  answers: buildAnswers(QUIZZES[0].questions, "correct"),        label: "emp1  → Fire Safety: PASS (100%)"       },
    { quizId: QUIZ_IDS.fireSafety,      videoId: VID_IDS[1], staffId: STAFF.emp2.id,  answers: buildAnswers(QUIZZES[0].questions, "wrong"),           label: "emp2  → Fire Safety: FAIL (0%)"         },
    { quizId: QUIZ_IDS.fireSafety,      videoId: VID_IDS[1], staffId: STAFF.admin.id, answers: buildAnswers(QUIZZES[0].questions, "correct"),         label: "admin → Fire Safety: PASS (100%)"       },
    { quizId: QUIZ_IDS.workplaceEthics, videoId: VID_IDS[2], staffId: STAFF.mgr1.id,  answers: buildAnswers(QUIZZES[1].questions, "mostly-correct"),  label: "mgr1  → Ethics: PASS (75%)"             },
    { quizId: QUIZ_IDS.workplaceEthics, videoId: VID_IDS[2], staffId: STAFF.emp3.id,  answers: buildAnswers(QUIZZES[1].questions, "half"),            label: "emp3  → Ethics: FAIL (50%)"             },
    { quizId: QUIZ_IDS.dataProtection,  videoId: VID_IDS[3], staffId: STAFF.emp4.id,  answers: buildAnswers(QUIZZES[2].questions, "correct"),         label: "emp4  → Data Protection: PASS (100%)"   },
    { quizId: QUIZ_IDS.dataProtection,  videoId: VID_IDS[3], staffId: STAFF.emp5.id,  answers: buildAnswers(QUIZZES[2].questions, "mostly-correct"),  label: "emp5  → Data Protection: FAIL (75%<80%)" },
  ];

  let created = 0;
  for (const a of attempts) {
    const quiz   = QUIZZES.find((q) => q.id === a.quizId)!;
    const score  = calcScore(quiz, a.answers);
    const passed = score >= quiz.passMark;

    await client.query(
      `INSERT INTO "VideoQuizAttempt"
         (id, "quizId", "videoId", "staffId", answers, score, passed, "attemptedAt")
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7)`,
      [a.quizId, a.videoId, a.staffId, JSON.stringify(a.answers), score, passed, randDaysAgo(10)]
    );

    const icon = passed ? "✅ PASS" : "❌ FAIL";
    log.ok(`[${icon} ${Math.round(score).toString().padStart(3)}%] ${a.label}`);
    created++;
  }

  const totalQ = QUIZZES.reduce((s, q) => s + q.questions.length, 0);
  log.info(`Quizzes: ${QUIZZES.length} quizzes, ${totalQ} questions, ${created} attempts`);
}
...
async function verifyCounts(): Promise<void> {
  ...
  const queries: Array<[string, string]> = [
    ...
    ["  → READY",                 `SELECT COUNT(*) FROM "HrVideo" WHERE "uploadedBy" = '${SEED_MARKER}' AND status = 'READY'`],
    ["  → PROCESSING",            `SELECT COUNT(*) FROM "HrVideo" WHERE "uploadedBy" = '${SEED_MARKER}' AND status = 'PROCESSING'`],
  ...
  for (const [label, sql] of queries) {
    try {
      ...
      console.log(`  ${c.dim}${label.padEnd(35)}${c.reset}${c.bold}${count}${c.reset}`);
    } catch {
      log.warn(`${label} — table not found`);
    }
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(`\n${c.bold}${c.cyan}═══════════════════════════════════════════`);
  console.log(`  ASKworX Module 7 — Seed Script (TypeScript)`);
  console.log(`═══════════════════════════════════════════${c.reset}`);
  console.log(`  Mode: ${[doClean && "CLEAN", doSeed && "SEED", doVerify && "VERIFY"].filter(Boolean).join(" + ")}\n`);
  ...
  if (doSeed) {
    // Run seeding inside a single transaction for atomicity
    const client = await pool.connect();
    try {
      await client.query("BEGIN");
      await seedStaff(client);
      await seedDocuments(client);
      await seedAcknowledgements(client);
      await seedVideos(client);
      await seedWatchLogs(client);
